  	 	...
  	 	$download.on("click", function(){
  	  		var imgUrl = $image.attr("src");
  	   		if(!imgUrl) {
		  	   	...
		  	   	var _url = __ctxPath + "/upd/dowload";
				var _path = $(".gallery").attr("data-pic");
				var _name = $(".gallery").attr("data-name");
				if(isNotBlank(_path)){
					_name = encodeURI(encodeURI($(".gallery").attr("data-name")));
					window.location= _url+"?filePath="+_path+"&fileName="+_name;
				}
  	   		}
  	  	});
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		$("#J_pg").remove();
		var _fullPicPathName = encodeURI(encodeURI($("#_fullPicPath").attr("data-name")));
		var _fullPicPath = $("#_fullPicPath").attr("src");
		$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", __ctxPath+"/gallery.jsp?decorator=blank&fullPicPathName="+_fullPicPathName+"&picPath="+_fullPicPath)
			.css({
				position : "absolute",
				'z-index' : 9999,
				left : 250,
